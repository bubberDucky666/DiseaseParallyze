...
# Try test where only one node can recieve edge lists
...
# MINS AND MAX VALUES NEED TO BE
	...
	for frame in range(duration):
		...
		for n in range(8):
			#if sNodes[n] == 3:
				#print("{})sent stuff to 3".format(rank))
			comm.send(ePartList[n], dest=sNodes[n], tag=0)
			#print("{}) sent to {}".format(rank, sNodes[n]))
		
		print("{}) Sent edge bois to edge nodes".format(rank))
		...
		for r in range(numNodes**2):
			...
			comm.send(extracts[r], dest=dest, tag=1)
